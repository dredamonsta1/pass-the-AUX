...
// Corrected import path for axiosInstance: using relative path
import axiosInstance from "../../utils/axiosInstance";
...
const AuthForm: FC = () => {
  ...
  // BASE_URL is not strictly needed here if axiosInstance is configured
  // const BASE_URL = "https://ninebyfourapi.herokuapp.com/api"; // Your backend base URL
  ...
  const handleAuth = async (): Promise<AxiosResponse<AuthResponseData>> => {
    ...
    if (isSignUp) {
      // Register
      // Use axiosInstance directly, relative to its baseURL
      const payload = {
        username: formData.username,
        email: formData.email,
        password: formData.password,
      ...
      response = await axiosInstance.post<AuthResponseData>("/users/register", payload);
    } else {
      // Login
      // Use axiosInstance directly, relative to its baseURL
      const payload = {
      ...
      response = await axiosInstance.post<AuthResponseData>("/users/login", payload);
    }
    return response; // Return the response object from the API call
  };
